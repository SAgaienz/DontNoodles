...
def Nonlinear_Model(y0, tspan, U, Ts, Tboil, T_evap_start, h):
    ...
    def nonlinear_mod(t, arr, args): 
        ...
        λ =  2396.4e3
        σ = 5.67e-8
        r_evap_max = R_evap(T, V, U(t), [Ts, Tboil, T_evap_start, h])[1]
        dm_vdt = R_evap(T, V, U(t), [Ts, Tboil, T_evap_start, h])[0]
        dVdt = -dm_vdt
        rad = (σ*A*((T**4) - (Ts**4)))
        # rad =0
        dTdt = (U(t) - (h*A*(T - Ts)) - rad - dm_vdt*λ)/(Cp*V)
    ...
    return [tspan, [U(t) for t in tspan], Tspan, Vspan, mspan]
...
tspan, Uspan, Tspan, Vspan, mspan = Nonlinear_Model([T0, V0, m_v0], tspan, U, Ts, Tboil, T_ev_s, h)
